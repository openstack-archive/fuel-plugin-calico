...
sudo sed -i "/^type_drivers/d" /etc/neutron/plugins/ml2/ml2_conf.ini
...
sudo sed -i "/^mechanism_drivers/d" /etc/neutron/plugins/ml2/ml2_conf.ini
...
sudo sed -i "/^tenant_network_types/d" /etc/neutron/plugins/ml2/ml2_conf.ini
...
sudo sed -i "/^dhcp_agents_per_network/d" /etc/neutron/neutron.conf
...
sudo sed -i "/^api_workers/d" /etc/neutron/neutron.conf
sudo sed -i "/^rpc_workers/d" /etc/neutron/neutron.conf
...
sudo sed -i "/^agent_down_time/d" /etc/neutron/neutron.conf
...
# Edit the /etc/calico/acl_manager.cfg file:
#
#     Change the PluginAddress to the host name or IP address of the
#     controller node. Then restart the ACL manager service.

cp /etc/calico/acl_manager.cfg.example /etc/calico/acl_manager.cfg

sudo sed -i "/^PluginAddress/d" /etc/calico/acl_manager.cfg
...
# Allow Calico API connections through the Fuel firewall
iptables -I INPUT 1 -p tcp --dport 9901 -j ACCEPT
iptables -I INPUT 1 -p tcp --dport 9903 -j ACCEPT
iptables -I INPUT 1 -p tcp --dport 9904 -j ACCEPT
iptables -I INPUT 1 -p tcp --dport 9905 -j ACCEPT
iptables -I INPUT 1 -p tcp --dport 9906 -j ACCEPT
